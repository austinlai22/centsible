...
export function RedeemList({points,confirming,setConfirming,handleRedeem}){
  return(
    <div style={{...S.col,gap:12}}>
      <div style={{background:"var(--sage-light)",borderRadius:12,padding:"12px 14px",border:"1px solid #c5dac3"}}>
        <p style={{fontSize:13,color:"var(--sage)",lineHeight:1.55}}>
      ...
      {CHARITIES.map(ch=>{
        ...
        return(
          <div key={ch.id} style={{border:"1px solid "+(isC?"#c5dac3":"var(--sand)"),borderRadius:14,padding:16,background:isC?"var(--sage-light)":"#fff",transition:"all .2s"}}>
            <div style={{...S.row,gap:13,alignItems:"flex-start"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{...S.between,gap:8,marginBottom:4,flexWrap:"wrap"}}>
                  ...
                  <Badge label={ch.cause} color="var(--sky)" colorLight="var(--sky-light)"/>
                </div>
                ...
                <div style={{marginTop:10,...S.between,gap:8,flexWrap:"wrap"}}>
                  ...
                  {!isC?(
                    <button onClick={()=>canAfford&&setConfirming(ch.id)} disabled={!canAfford}
                      style={S.btn(canAfford?"#1A1714":"var(--sand)",canAfford?"#fff":"var(--stone)",{cursor:canAfford?"pointer":"default",minHeight:36})}>
                      {canAfford?"Donate":"Need more pts"}
                    </button>
                  ):(
                    <div style={{...S.row,gap:8}}>
                      <button onClick={()=>setConfirming(null)} style={S.sandBtn({padding:"8px 12px",minHeight:36})}>Cancel</button>
                      <button onClick={()=>handleRedeem(ch)} style={S.btn("var(--sage)","#fff",{padding:"8px 14px",minHeight:36})}>Confirm →</button>
                    </div>
                  )}
                </div>
              </div>
    ...
    <div style={{...S.col,gap:18}}>
      <div style={{background:"#1A1714",borderRadius:20,padding:"28px 26px 24px",color:"#F5F0E8",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{...S.row,gap:14,marginBottom:18,position:"relative"}}>
          ...
          <div>
            <p style={{fontSize:11,color:"#C8BAA8",textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>Your level</p>
            <p style={{...S.serif,fontSize:26,fontWeight:400}}>{cur.name}</p>
          </div>
        </div>
        <div style={{...S.row,alignItems:"baseline",gap:6,marginBottom:16,position:"relative"}}>
          <span style={{...S.serif,fontSize:42,fontWeight:300,color:"#B8882A"}}>{points}</span>
          <span style={{fontSize:15,color:"#C8BAA8"}}>points</span>
        </div>
        {next?(
          <div style={{position:"relative"}}>
            <div style={{...S.between,marginBottom:7,gap:8}}>
              <span style={{fontSize:12,color:"#C8BAA8"}}>Next: {next.icon} {next.name}</span>
              <span style={{fontSize:12,color:"#B8882A",fontWeight:600}}>{next.min-points} pts to go</span>
            </div>
            <div style={{height:6,background:"rgba(255,255,255,.1)",borderRadius:4,overflow:"hidden"}}>
              <div style={{height:"100%",width:prog+"%",background:"#B8882A",borderRadius:4,transition:"width .6s ease"}}/>
            </div>
          </div>
        ):<p style={{fontSize:13,color:"#B8882A",fontWeight:500,position:"relative"}}>🔥 Maximum level — you're a Flow Master!</p>}
      </div>
        ...
        {LEVELS.map((l,i)=>{
          ...
          return(
            <div key={l.level} style={{...S.row,padding:"10px 0",borderBottom:i<LEVELS.length-1?"1px solid var(--sand)":"none"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <p style={{fontSize:14,fontWeight:isCur?600:400,color:reached?"var(--ink)":"var(--stone)"}}>{l.name}</p>
      ...
      <Card>
        ...
        {EARN_ACTIONS.map((a,i)=>(
          <div key={a.key} style={{...S.between,padding:"10px 0",gap:12,borderBottom:i<EARN_ACTIONS.length-1?"1px solid var(--sand)":"none"}}>
        ...
        <button onClick={async()=>{try{await earn("complete_monthly_review");}catch(e){alert(e.message);}}}
          style={{marginTop:16,width:"100%",background:"var(--gold-light)",border:"1px solid var(--gold)",borderRadius:10,padding:"12px",fontSize:13,fontWeight:600,cursor:"pointer",color:"var(--gold)",minHeight:44}}>
        ...
        <Card>
          ...
          {redeemed.map((r,i)=>(
            <div key={r.id} style={{...S.row,padding:"10px 0",borderBottom:i<redeemed.length-1?"1px solid var(--sand)":"none"}}>
              ...
              <span style={{fontSize:13,fontWeight:600,color:"var(--sage)",whiteSpace:"nowrap"}}>−{r.pts} pts</span>
            </div>
          ))}
        </Card>
      )}

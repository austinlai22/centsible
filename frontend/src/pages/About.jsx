    ...
    <>
      <div className="slide-up" style={{...S.col,gap:4}}>
        <div style={{background:"#1A1714",borderRadius:20,padding:"24px 22px",color:"#F5F0E8",...S.row,gap:16,marginBottom:8}}>
          <div style={{width:54,height:54,borderRadius:"50%",background:"#B8882A",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:700,flexShrink:0,color:"#1A1714"}}>
            {(profile.name||"?").charAt(0).toUpperCase()}
          </div>
          <div style={{flex:1,minWidth:0}}>
            <p style={{...S.serif,fontSize:22,fontWeight:400}}>{profile.name||"Your account"}</p>
            <p style={{fontSize:13,color:"#C8BAA8",marginTop:2}}>{cur.icon} {cur.name} · {points} pts</p>
          </div>
        </div>
        ...
        <Card style={{padding:"0 20px"}}>
          ...
          <div style={{padding:"12px 0 6px",borderBottom:"1px solid var(--sand)"}}>
            ...
            <div style={{height:5,background:"var(--sand)",borderRadius:4,overflow:"hidden"}}>
              <div style={{height:"100%",width:prog+"%",background:"var(--gold)",borderRadius:4}}/>
            </div>
          </div>
        ...
        <button onClick={onLogout} style={{background:"#fff",border:"1px solid var(--sand)",borderRadius:14,padding:"15px",fontSize:15,fontWeight:500,cursor:"pointer",color:"var(--rose)",display:"flex",alignItems:"center",justifyContent:"center",gap:8,minHeight:48}}>
          ⏻ Log out
        </button>
      ...
      {section==="accounts"&&(
        <Sheet title="Linked accounts" subtitle="Powered by Plaid — we never see your credentials" onClose={close}>
          <div style={{...S.col,padding:"16px 20px calc(36px + env(safe-area-inset-bottom))",overflowY:"auto",flex:1,minHeight:0}}>
            ...
            {accountsLoading && !accounts?.length ? (
              ...
              <>
                ...
                {(accounts||[]).map(a=>{
                  ...
                  return(
                    <div key={a.id} style={{border:"1px solid var(--sand)",borderRadius:13,padding:"14px 16px",...S.between,gap:10,opacity:isRemoving?.5:1,transition:"opacity .2s"}}>
                      ...
                      <button onClick={()=>handleRemoveAccount(a)} disabled={isRemoving} style={S.sandBtn({background:"var(--rose-light)",color:"var(--rose)",flexShrink:0,minHeight:34})}>
            ...
            {linkError && (
              <div role="alert" style={{background:"var(--rose-light)",border:"1px solid #e8b4b2",borderRadius:12,padding:"12px 14px"}}>
